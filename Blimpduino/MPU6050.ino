...
// Default I2C address for the MPU-6050 is 0x68. MPU-9250 is 0x69
byte MPU6050_I2C_ADDRESS = 0x69;
...
float MPU6050_yawAngle(float dt)
{
...
void MPU6050_setYawAngle(float newYawAngle)
{
  yawAngle = newYawAngle;
}

float MPU6050_getYawAngle(void)
{
  return yawAngle;
}
...
void MPU6050_setup()
{
  ...
  MPU6050_write_reg(MPU6050_SMPLRT_DIV, 19);  // 50Hz : Sample Rate = 1000 / (1 + SMPLRT_DIV) Hz
  // Set Sample Rate to 20Hz
  //MPU6050_write_reg(MPU6050_SMPLRT_DIV, 49);  // 20Hz : Sample Rate = 1000 / (1 + SMPLRT_DIV) Hz
...
void MPU6050_read_3axis()
{
  ...
  if (error != 0) {
    SerialUSB.print("MPU6050 Error:");
    SerialUSB.println(error);
    SerialUSB.print("Changing I2C address to 0x68..");
...
void MPU6050_read_1axis()
{
  ...
  SerialUSB.print(accel_t_gyro.value.y_accel, DEC);
  SerialUSB.print(",");
  SerialUSB.println(accel_t_gyro.value.x_gyro, DEC);
}
